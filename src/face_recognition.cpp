...
* A ROS simple actionlib server for object recognition in video stream. It provides different object recognition functionalities such as adding training images directly from the video stream, re-training (updating the database to include new training images), recognizing objects in the video stream, etc.

* All image processing and object recognition functionalities are provided by utilizing the Shervin Emami's c++ source code for object recognition (http://www.shervinemami.info/objectRecognition.html). Object Recognition is performed using Eigenobjects (also called "Principal Component Analysis" or PCA) 
...
#include <object_recognition/ObjectRecognitionAction.h>
...
//#include <term.h>
#include <unistd.h>
#include "object_recognition_lib.cpp"
...
class ObjectRecognition
{
    ...
    ObjectRecognition(std::string name) : 
    frl(),
    it_(nh_),
    pnh_("~"),
    as_(nh_, name, boost::bind(&ObjectRecognition::executeCB, this, _1), false)
    {
    //a object recognized with confidence value higher than the confidence_value threshold is accepted as valid.
        ...
        //a parameter for the "add_object_images" goal which determines the number of training images for a new object (person) to be acquired from the video stream 
        pnh_.param<int>("add_object_number", add_object_number, 25);
        add_object_number = 25;
        ...
        if(calcNumTrainingPerson("train.txt")!=frl.nPersons)
        {
            ...
            ROS_WARN("Alert: Database is not updated. Please delete \"objectdata.xml\" and re-run!"); //AFTER
        }
        //subscribe to video stream through image transport class
        image_sub_ = it_.subscribe("/camera/image_raw", 1, &ObjectRecognition::imageCB, this);
        image_pub = it_.advertise("/object_recognition/image", 1);
    }
    ...
    void executeCB(const object_recognition::ObjectRecognitionGoalConstPtr &goal)
    {
    //check to be sure if the goal should be still persuaded
        ...
        //check if the name of the person has been provided for the add-object-images goal  
        ...
        { 
          //(exit) Goal is to exit
            ...
            case 1:
              //(add_object_images) Goal is to take a number of(add_object_number) images of a person's object from the video stream and save them as training images 
            case 2: 
      	    {
            	if(goal_id_ == 2)
                add_object_count=0;
    ...
    void imageCB(const sensor_msgs::ImageConstPtr& msg)
    {
        ...
        CvRect objectRect;
        // Make sure the image is greyscale, since the Eigenobjects is only done on greyscale image.
        greyImg = frl.convertImageToGreyscale(img);
        // Perform object detection on the input image, using the given Haar cascade classifier.
        objectRect = frl.detectObjectInImage(greyImg,frl.objectCascade);
        // Make sure a valid object was detected.
        if (objectRect.width < 1) 
        {
            ROS_INFO("No object was detected in the last frame"); 
            if(show_screen_flag)
            {
                cvPutText(img, text_image.str().c_str(), cvPoint(10, objectRect.y + 50), &font, textColor);
        ...
        cvRectangle(img, cvPoint(objectRect.x, objectRect.y), cvPoint(objectRect.x + objectRect.width-1, objectRect.y + objectRect.height-1), CV_RGB(0,255,0), 1, 8, 0);
        objectImg = frl.cropImage(greyImg, objectRect);	// Get the detected object image.
        // Make sure the image is the same dimensions as the training images.
        sizedImg = frl.resizeImage(objectImg, frl.objectWidth, frl.objectHeight);
        // Give the image a standard brightness and contrast, in case it was too dark or low contrast.
        equalizedImg = cvCreateImage(cvGetSize(sizedImg), 8, 1);	// Create an empty greyscale image
        cvEqualizeHist(sizedImg, equalizedImg);
        cvReleaseImage( &greyImg );cvReleaseImage( &objectImg );cvReleaseImage( &sizedImg );  
        ...
        //goal is add_object_images
        if( goal_id_== 2)      
        {
            if(add_object_count==0)  
            {
             //assign the correct number for the new person
                person_number = calcNumTrainingPerson("train.txt")+1; 
            }
            char cstr[256];
    	    sprintf(cstr, "data/%d_%s%d.pgm", person_number, &goal_argument_[0], add_object_count+1);
            ROS_INFO("Storing the current object of '%s' into image '%s'.", &goal_argument_[0], cstr);
            ...
            //get from parameter server how many training imaged should be acquire.
                ros::param::getCached("~add_object_number", add_object_number);
                if(add_object_number<=0)
                {
                    ROS_INFO("add_object_number parameter is Zero, it is Invalid. One object was added anyway!");
                    add_object_number=1;
                } 
                frl.database_updated = false;
            }
            ...
            if(++add_object_count==add_object_number)	
        	{
        ...
        if( goal_id_<2 )      
        {
            ...
            projectedTestObject = (float *)cvAlloc( frl.nEigens*sizeof(float) );
            // project the test image onto the PCA subspace
            cvEigenDecomposite(equalizedImg,frl.nEigens,frl.eigenVectArr,0, 0,frl.pAvgTrainImg,projectedTestObject);
...
protected:
    ...
    int goal_id_;
    object_recognition::ObjectRecognitionFeedback feedback_; 
    object_recognition::ObjectRecognitionResult result_;
    int add_object_count; //help variable to count the number of training images already taken in the add_object_images goal
    FILE *trainFile; 
    double confidence_value;//a object recognized with confidence value higher than confidence_value threshold is accepted as valid.
    bool   show_screen_flag;//if output window is shown
    int    add_object_number; //the number of training images to be taken in add_object_images goal
    CvFont font;
    CvScalar textColor;
    ostringstream text_image;
    ros::NodeHandle nh_, pnh_;
    ObjectRecognitionLib frl; 
    image_transport::ImageTransport it_;
    image_transport::Subscriber image_sub_;
    image_transport::Publisher image_pub;
    actionlib::SimpleActionServer<object_recognition::ObjectRecognitionAction> as_;
...
int main(int argc, char** argv)
{
    ...
    ObjectRecognition object_recognition(ros::this_node::getName());
    ros::spin();
    return 0;
}
